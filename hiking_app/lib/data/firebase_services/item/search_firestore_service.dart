...
import 'package:hiking_app/domain/models/item/gear_item.dart'; // Import your existing Item model
...
class SearchFirestoreService {
  ...
  Future<void> saveSearchTerm(String term) async {
    ...
    if (existingSearches.docs.isNotEmpty) {
      ...
      await existingSearches.docs.first.reference
  ...
  Future<List<Item>> searchGearItems(String keyword) async {
    final gearSnapshot = await _firestore
        .collection('gear_items')
        .get(); // You might want to add .where() clauses for better performance if possible
  ...
  Future<List<Map<String, dynamic>>> searchShops(String keyword) async {
    final shopSnapshot = await _firestore
        .collection('shops')
        .get(); // Consider adding indexes or full-text search for large datasets

    final shopMatches = shopSnapshot.docs
        .where(
          (doc) => (doc['name_lowercase'] ??
                  '') // Assuming 'name_lowercase' for shops as well
              .toString()
              .contains(keyword.toLowerCase()),
        )
        .map((doc) => doc.data())
