...
import 'package:hiking_app/domain/models/item/gear_item.dart'; // Import your Item model
  ...
  Future<Item?> getGearItem(String itemId) async {
    try {
      DocumentSnapshot doc =
          await _db.collection('gear_items').doc(itemId).get();
      if (doc.exists) {
  ...
  Future<bool> addItemToCart({
    ...
    try {
      await _db.runTransaction((transaction) async {
        ...
        int currentAvailableQtyInDb = (latestGearItemSnapshot.data()
        ...
        if (existingCartItemQuery.docs.isNotEmpty) {
          ...
          transaction.set(cartItemsRef.doc(),
              cartItemData); // Use set with auto-generated ID
