...
import 'package:hiking_app/domain/models/item/cart_item.dart'; // Import your CartItem model
...
class CartFirestoreService {
  ...
  Future<void> updateCartItemQuantityAndStock(String userId, String cartItemId,
      String gearItemId, int newQuantity) async {
    // If new quantity is less than 1, implicitly remove the item
    if (newQuantity < 1) {
      await removeCartItemAndReturnStock(userId, cartItemId, gearItemId,
          1); // Assuming 1 unit is being "removed"
      return;
    }
    ...
    await _firestore.runTransaction((transaction) async {
      DocumentSnapshot latestCartItemSnapshot =
          await transaction.get(cartItemDocRef);
      DocumentSnapshot latestGearItemSnapshot =
          await transaction.get(gearItemDocRef);
      ...
      int currentQuantityInCart =
          (latestCartItemSnapshot.data() as Map<String, dynamic>)['quantity'] ??
              0;
      int currentAvailableQtyInGearItem = (latestGearItemSnapshot.data()
      ...
      if (quantityChange > 0) {
        ...
        transaction.update(gearItemDocRef, {
          'available_qty':
              FieldValue.increment(-quantityChange), // -change makes it +
        });
      }
  ...
  Future<void> removeCartItemAndReturnStock(String userId, String cartItemId,
      String gearItemId, int quantityInCart) async {
    final cartItemDocRef = _userCartItemsCollection(userId).doc(cartItemId);
    final gearItemDocRef = _firestore.collection('gear_items').doc(gearItemId);
  ...
  Future<void> clearUserCartAndReturnStock(String userId) async {
    ...
    for (var doc in cartItemsSnapshot.docs) {
      ...
      if (gearItemId.isNotEmpty && quantityInCart > 0) {
        DocumentReference gearItemRef =
            _firestore.collection('gear_items').doc(gearItemId);
        batch.update(gearItemRef,
            {'available_qty': FieldValue.increment(quantityInCart)});
      }
  ...
  Future<void> createOrder(
      String userId, List<CartItem> items, double orderTotal) async {
    try {
      await _firestore.collection('orders').add({
        'userId': userId,
        'items': items
            .map((item) => item.toFirestore())
            .toList(), // Convert cart items to Firestore maps
